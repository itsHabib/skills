...
Four commands, in this exact shape — `@claude review`, `@codex review`, and `@cursor review` as separate comments, Copilot as a requested reviewer. Bots take the literal trigger word `review` after the mention; bare `@claude` / `@codex` / `@cursor` is unreliable. Do NOT comment `@copilot` (it pushes commits).
...
Cursor Bugbot also auto-fires as a CI check (`Cursor Bugbot: SUCCESS`) on PRs in repos with the GitHub App installed; the explicit `@cursor review` ping adds a richer review pass posted as a `cursor[bot]` comment. If the Copilot reviewer add fails (handle differs per repo/org), retry with `copilot-pull-request-reviewer`. If both fail, print a one-liner asking the user to add Copilot manually and continue — don't block on this.
