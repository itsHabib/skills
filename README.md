...
| Skill | What it does |
|---|---|
| `/boost` | Unblock engineering, algorithm and research work with focused experiments, side-agent missions and a verified handback; includes questions for steering stuck investigations. |
...
| `/hackathon` | Run a build competition where the judge re-runs everything: one isolated builder per idea on a distinct domain x archetype cell, a facts-only `RESULT.json` contract, a hard barrier, then a judge that re-runs every test and demo from clean, applies an honesty multiplier to self-reports, and proposes PROMOTE / SEED / PARK / DROP on one scoreboard you ratify. Standalone repos or one worktree per entry inside an existing repo. |
