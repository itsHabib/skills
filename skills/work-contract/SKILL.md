...
argument-hint: "[create|refresh|validate|close] <task or WORK.md path>"
user_invocable: true
---
...
It also rejects an empty list item. Keep each item's text on its dash line: a
bare `-` counts as empty even when its text continues on the next line.
