...
- Confirm the repo (the cwd's project, under your portfolio directory).
- Resolve the dossier project: `mcp__dossier__project_get { slug: <project> }`.
  - Exists → use it.
  - Not found AND this is a new initiative → propose `mcp__dossier__project_create { slug, title, description, actor: "claude-code" }`. Confirm the slug/title with the operator before creating — a project is the top-level unit and the slug is immutable.
